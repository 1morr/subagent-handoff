...
- **GUI 可讀性的幾處細節。** 等寬字堆疊在 `monospace` 之前補上漢字字型：「`.num` 不准套在中文上」這條規則
  ...
  色塊（之前只有一個小小的 OFF，看起來仍在把流量送去 provider）。預覽結果與進條批註的「命中」印「第 2 條」
  而不是內部 id `r-azi7us5z`。接入分頁上 `CLAUDE_CODE_ATTRIBUTION_HEADER` 那段會讓輔助請求全部 429 的
  警告，從灰色小字改成提示框。

- **GUI 的對比與無障礙：五個分頁的 Lighthouse 無障礙分數都到 100。** 分頁列補上 `tablist`／`tab`／`tabpanel`
  （原本只有 `aria-selected`，沒有角色就不會被念出來）；規則列的勾選框、兩個下拉選單與兩個輸入框補上名稱
  （原本只有 title 或 placeholder）；存檔狀態與 toast 改成 live region。對比不足的五處：預覽結果的欄名
  2.80:1、PRV 標題列上的 provider id 3.28:1、流量告警橫幅的欄名 3.63:1、規則序號格的 ON／OFF 4.16:1、
  紅框按鈕的 11px 字 4.38:1（改用 `--alarm-ink`，照 ui-notes 的規則），全部調到 4.5:1 以上。

- **390px 寬的手機版面不再整頁橫向捲動。** 機架與流量的欄寬是固定的（實測 scrollWidth 938），現在由機架
  自己的容器橫向捲動；席位卡的最小寬度 420px 改成不超過容器寬；分頁列在窄螢幕不換行、改為橫向捲動
  （原本「接入」會單獨掉到第二列）；頁首的語言標籤跟選單綁在一起，不會一個在行尾、一個在下一行。
...
- **接入分頁講清楚：打開 HTTPS proxy 模式不影響原本用 `ANTHROPIC_BASE_URL` 的 CLI。** 模式只是在同一個埠上
  多一個 `CONNECT` 入口，明文的 `/v1/messages*` 照舊（開啟狀態下實測）。之前開啟時第 1 步整個換成
  `HTTPS_PROXY` 片段、切換提醒要人「拿掉 `ANTHROPIC_BASE_URL`」、狀態列寫「CLI 與 Desktop 透過
  `HTTPS_PROXY` 接進來」，讀起來像是一打開 CLI 就得跟著改（使用者實際這樣問）。現在三個步驟在兩種模式下
  都是 `ANTHROPIC_BASE_URL` 的接法；Desktop 的片段、「只用 CLI／兩者都走／只在某個 repo」三種放法的表、
  同一套設定不能兩個都設的警告，只在模式開著時出現在最下面的面板；原理說明的流程圖與比較表也補上
  「CLI 照舊」那一條。
